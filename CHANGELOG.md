...
- **README — `@capFrame` rendered HTML example corrected.** The "This renders:"
  block in the "Iframe mode" section still reflected the pre-v1.8.4 state
  (visible iframe with `width:300px;height:58px;`, no `aria-hidden`, spurious
  `loading="lazy"`) and the pre-v1.8.7 state (no `e.source` check against the
  iframe's `contentWindow` in the parent-side `message` listener). The block
  now matches the output of `@capFrame` without arguments as generated by
  `CapServiceProvider` today. The companion Blade example and the nonce/no-nonce
  note were adjusted accordingly.
